      ...
      {unread > 0 && (
        <span
          ...
          style={{
            ...
            animation: unread > 0 ? "fadeInUp 0.2s ease" : "none",
          }}
        >
          {unread > 99 ? "99+" : unread}
        </span>
...
export function NotificationPanel({ onClose, onUnreadChange }: Props) {
  ...
  const handleMarkRead = async (n: AppNotification) => {
    if (n.is_read) return;
    await markRead(n.id).catch(() => { /* silent */ });
  ...
  const handleMarkAllRead = async () => {
    setMarkingAll(true);
    await markAllRead().catch(() => { /* silent */ });
