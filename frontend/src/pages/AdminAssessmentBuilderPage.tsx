...
function parseApiError(err: unknown, fallback: string): string {
  ...
  if (jsonStart !== -1) {
    try {
      const payload = JSON.parse(err.message.slice(jsonStart));
    ...
    async function load() {
      try {
        ...
        if (assessments.length > 0) {
          ...
          // Load questions (admin view includes is_correct)
  ...
  const updateOption = (optIdx: number, field: keyof OptionDraft, value: string | boolean) => {
    setQDraft((prev) => ({
      ...
      options: prev.options.map((o, i) => {
        if (i !== optIdx) {
          // If setting is_correct = true on this option, clear others
  ...
  return (
    <div className="page-shell">
      ...
      <main className="page-content page-content--narrow page-enter">
        ...
        <button className="back-btn" onClick={() => navigate(`/admin/content/courses/${courseId}/lessons`)}>
        ...
        {loading ? (
          <div>
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="skeleton" style={{ height: 52, borderRadius: "var(--radius-md)", marginBottom: "var(--space-3)" }} />
            ))}
          </div>
        ) : (
          <>
            ...
            <div className="card" style={{ marginBottom: "var(--space-6)" }}>
              <div className="form-group">
                <label className="form-label" htmlFor="a-title">Assessment Title *</label>
                <input
                  id="a-title"
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-4)" }}>
                <div className="form-group">
                  <label className="form-label" htmlFor="pass-marks">Pass Marks</label>
                  <input
                    id="pass-marks"
                    className="form-input"
                    type="number"
              ...
              {assessment && (
                <div style={{
                  ...
                  <span>Questions: <strong style={{ color: "var(--text-primary)" }}>{questions.length}</strong></span>
                  <span>Pass: <strong style={{ color: "var(--success)" }}>{aPassMarks}</strong></span>
                </div>
              )}
              ...
              <>
                ...
                {questions.length === 0 && !showQForm ? (
                  <div className="empty-state" style={{ padding: "var(--space-8)" }}>
                    ...
                    <p className="empty-state__message">Add questions to this assessment.</p>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)", marginBottom: "var(--space-6)" }}>
                    {questions.map((q, i) => (
                      <div key={q.id} className="card page-enter" style={{ animationDelay: `${i * 50}ms` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "var(--space-3)" }}>
                          <div>
                            <div style={{ fontSize: "var(--text-xs)", color: "var(--brand-primary)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "var(--space-1)" }}>
                              Q{q.order} · {q.marks} {q.marks === 1 ? "mark" : "marks"}
                            </div>
                          ...
                          <button
                            ...
                            style={{ padding: "var(--space-1) var(--space-2)", color: "var(--error)", fontSize: "var(--text-xs)", flexShrink: 0 }}
                          ...
                          {q.options.map((opt) => (
                            <div
                              ...
                              style={{
                                ...
                                borderRadius: "var(--radius-sm)",
                                background: opt.is_correct ? "rgba(63,185,80,0.08)" : "var(--bg-elevated)",
                                border: `1px solid ${opt.is_correct ? "rgba(63,185,80,0.3)" : "var(--border-subtle)"}`,
                                fontSize: "var(--text-sm)",
                ...
                {showQForm && (
                  <div className="card" style={{ marginBottom: "var(--space-6)" }}>
                    <h4 style={{ fontFamily: "var(--font-display)", fontSize: "var(--text-base)", fontWeight: 700, color: "var(--text-primary)", marginBottom: "var(--space-4)" }}>
                      New Question
                    </h4>
                    ...
                    <div className="form-group">
                      ...
                      <textarea
                        ...
                        value={qDraft.text}
                        onChange={(e) => setQDraft((p) => ({ ...p, text: e.target.value }))}
                        rows={3}
                        style={{ resize: "vertical" }}
                      />
                    </div>
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        min={1}
                        value={qDraft.marks}
                        onChange={(e) => setQDraft((p) => ({ ...p, marks: Number(e.target.value) }))}
                        style={{ maxWidth: 120 }}
                      />
                    </div>
