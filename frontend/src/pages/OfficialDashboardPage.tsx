...
function StatCard({ label, value, accent, icon }: {
  ...
  return (
    <div className="card" style={{
      ...
      {icon && (
        <div style={{ fontSize: 20, marginBottom: "var(--space-1)", lineHeight: 1 }}>{icon}</div>
      )}
      <div style={{
    ...
    <>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <StatCard icon="🏫" label="Schools"       value={institutions.length}  accent="var(--role-official)" />
          <StatCard icon="🏛️" label="Total Classes"  value={classes.length}       accent="var(--ink-primary)" />
          <StatCard icon="👨‍🎓" label="Total Students" value={totalStudents}         accent="var(--role-student)" />
          <StatCard
            icon="📊"
            label="Avg Pass Rate"
            value={`${avgPassRate}%`}
          ...
          />
          <StatCard icon="📚" label="Active Courses" value={courses.length}        accent="var(--saffron)" />
        </div>
      )}
      ...
      <SectionHeader
        ...
        subtitle="Click any class to view student breakdown · filter by school to drill down"
      />
          ...
          {filteredClasses.map((c, i) => {
            ...
            return (
              <div
                ...
                onClick={() => navigate(`/official/classes/${c.class_id}`)}
                onMouseEnter={(e) => {
                ...
                role="button"
                tabIndex={0}
                aria-label={`View class ${c.class_name} details`}
                onKeyDown={(e) => e.key === "Enter" && navigate(`/official/classes/${c.class_id}`)}
              >
                <div style={{
                  fontSize: "var(--text-xs)",
                  color: "var(--ink-muted)",
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "var(--text-lg)", color: "var(--ink-primary)" }}>
                      {c.total_students}
                    </div>
                    <div style={{ fontSize: "var(--text-xs)", color: "var(--ink-muted)" }}>Students</div>
                  </div>
                  <div>
                    <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "var(--text-lg)", color: passColor }}>
                      {c.pass_rate}%
                    </div>
                    <div style={{ fontSize: "var(--text-xs)", color: "var(--ink-muted)" }}>Pass Rate</div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--role-official)", fontWeight: 600 }}>
                    View students →
                  </span>
                </div>
              </div>
